...
const HomePage: React.FC = () => {
  ...
  const { businesses, loading: businessesLoading } = useBusinessData();
  const { blogPosts, loading: blogLoading } = useBlogData();
  ...
  const handleNewsletterSubmit = (e: React.FormEvent) => {
    ...
    setNewsletterError('');
    console.log(`Subscribing email to newsletter: ${newsletterEmail}`);
  ...
  const featuredDeals = businesses
  ...
  const renderSection = (section: any) => {
    ...
    switch (section.type) {
      ...
      case 'featuredDeals':
        if (businessesLoading || featuredDeals.length === 0) return null;
        return (
      ...
      case 'featuredBlog':
        if (blogLoading || featuredPosts.length === 0) return null;
        return (
          <section key={section.id} className="py-16 bg-white">
            <div className="container mx-auto px-4">
              <h2 className="text-3xl font-bold text-center font-serif text-neutral-dark mb-2">{section.title}</h2>
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                {FEATURED_LOCATIONS.map((location) => (
                  <Link key={location.name} to={`/directory?location=${encodeURIComponent(location.name)}`} className="group relative rounded-lg overflow-hidden shadow-lg aspect-[3/4]">
                    <img src={location.imageUrl} alt={location.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" loading="lazy" />
    ...
    <div>
      ...
      <section className="relative h-[500px] overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 h-full flex flex-col justify-center items-center text-white text-center z-10">
          {heroSlides.length > 0 && currentSlide < heroSlides.length && (
            <>
            ...
            {isSubscribed ? (
              ...
              <form onSubmit={handleNewsletterSubmit} className="flex flex-col sm:flex-row gap-2">
                ...
                <input
                  ...
                  className="flex-grow w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary text-neutral-dark"
                />
                <button
                  type="submit"
                  className="px-6 py-3 bg-primary text-white font-semibold rounded-md hover:bg-primary-dark transition-colors flex-shrink-0 shadow-sm"
                >
                  Đăng ký
                </button>
              </form>
            )}
            {newsletterError && <p className="text-red-500 text-xs mt-2">{newsletterError}</p>}
          </div>
        </div>
      </section>
