...
-- ============================================
-- Function: Get user email from auth.jwt() (Faster and avoids querying auth.users table during RLS)
CREATE OR REPLACE FUNCTION public.get_user_email() RETURNS TEXT AS $$ BEGIN RETURN COALESCE(
...
CREATE POLICY "profiles_update_own_or_admin" ON public.profiles FOR
UPDATE USING (
    id = auth.uid()
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "profiles_delete_admin" ON public.profiles FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- BUSINESSES POLICIES
-- ============================================
-- SELECT: Public can read active businesses, owners can read their own (even if inactive), admins can read all
CREATE POLICY "businesses_select_public_active_or_owner_or_admin" ON public.businesses FOR
SELECT USING (
    is_active = TRUE
...
CREATE POLICY "businesses_insert_admin" ON public.businesses FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- INSERT: Authenticated users can create businesses where they are the owner (for onboarding wizard)
CREATE POLICY "businesses_insert_owner" ON public.businesses FOR
INSERT WITH CHECK (
    auth.uid() IS NOT NULL
    AND owner_id = auth.uid()
  );
-- UPDATE: Business owners can update their own business, admins can update any
CREATE POLICY "businesses_update_owner_or_admin" ON public.businesses FOR
UPDATE USING (
    owner_id = auth.uid()
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "businesses_delete_admin" ON public.businesses FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- SERVICES POLICIES
-- ============================================
-- SELECT: Public can read services of active businesses, owners can read their own, admins can read all
CREATE POLICY "services_select_public_or_owner_or_admin" ON public.services FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "services_insert_owner_or_admin" ON public.services FOR
INSERT WITH CHECK (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  );
-- UPDATE: Business owners can update their services, admins can update any
CREATE POLICY "services_update_owner_or_admin" ON public.services FOR
UPDATE USING (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "services_delete_owner_or_admin" ON public.services FOR DELETE USING (
  public.is_business_owner(auth.uid(), business_id)
  OR public.is_admin(public.get_user_email())
);
-- ============================================
...
CREATE POLICY "deals_select_public_or_owner_or_admin" ON public.deals FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
-- INSERT: Business owners can insert for their business, admins can insert for any
CREATE POLICY "deals_insert_owner_or_admin" ON public.deals FOR
INSERT WITH CHECK (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  );
-- UPDATE: Business owners can update their deals, admins can update any
CREATE POLICY "deals_update_owner_or_admin" ON public.deals FOR
UPDATE USING (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "deals_delete_owner_or_admin" ON public.deals FOR DELETE USING (
  public.is_business_owner(auth.uid(), business_id)
  OR public.is_admin(public.get_user_email())
);
-- ============================================
...
CREATE POLICY "team_members_select_public_or_owner_or_admin" ON public.team_members FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
-- INSERT: Business owners can insert for their business, admins can insert for any
CREATE POLICY "team_members_insert_owner_or_admin" ON public.team_members FOR
INSERT WITH CHECK (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  );
-- UPDATE: Business owners can update their team members, admins can update any
CREATE POLICY "team_members_update_owner_or_admin" ON public.team_members FOR
UPDATE USING (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "team_members_delete_owner_or_admin" ON public.team_members FOR DELETE USING (
  public.is_business_owner(auth.uid(), business_id)
  OR public.is_admin(public.get_user_email())
);
-- ============================================
...
CREATE POLICY "media_items_select_public_or_owner_or_admin" ON public.media_items FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
-- INSERT: Business owners can insert for their business, admins can insert for any
CREATE POLICY "media_items_insert_owner_or_admin" ON public.media_items FOR
INSERT WITH CHECK (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  );
-- UPDATE: Business owners can update their media, admins can update any
CREATE POLICY "media_items_update_owner_or_admin" ON public.media_items FOR
UPDATE USING (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "media_items_delete_owner_or_admin" ON public.media_items FOR DELETE USING (
  public.is_business_owner(auth.uid(), business_id)
  OR public.is_admin(public.get_user_email())
);
-- ============================================
...
CREATE POLICY "reviews_select_public_visible_or_own_or_owner_or_admin" ON public.reviews FOR
SELECT USING (
    (
      status = 'Visible'
      AND EXISTS (
...
CREATE POLICY "reviews_insert_authenticated_or_admin" ON public.reviews FOR
INSERT WITH CHECK (
    auth.uid() IS NOT NULL
    OR public.is_admin(public.get_user_email())
  );
...
CREATE POLICY "reviews_update_own_or_owner_or_admin" ON public.reviews FOR
UPDATE USING (
    user_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY "reviews_delete_own_or_admin" ON public.reviews FOR DELETE USING (
  user_id = auth.uid()
  OR public.is_admin(public.get_user_email())
);
-- ============================================
...
CREATE POLICY "blog_posts_insert_admin" ON public.blog_posts FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- UPDATE: Only admins can update blog posts
CREATE POLICY "blog_posts_update_admin" ON public.blog_posts FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete blog posts
CREATE POLICY "blog_posts_delete_admin" ON public.blog_posts FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
...
CREATE POLICY "business_blog_posts_select_public_published_or_owner_or_admin" ON public.business_blog_posts FOR
SELECT USING (
    (
      status = 'Published'
      AND EXISTS (
...
-- INSERT: Business owners can insert for their business, admins can insert for any
CREATE POLICY "business_blog_posts_insert_owner_or_admin" ON public.business_blog_posts FOR
INSERT WITH CHECK (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  );
-- UPDATE: Business owners can update their posts, admins can update any
CREATE POLICY "business_blog_posts_update_owner_or_admin" ON public.business_blog_posts FOR
UPDATE USING (
    public.is_business_owner(auth.uid(), business_id)
    OR public.is_admin(public.get_user_email())
  ) WITH CHECK (
...
CREATE POLICY "business_blog_posts_delete_owner_or_admin" ON public.business_blog_posts FOR DELETE USING (
  public.is_business_owner(auth.uid(), business_id)
  OR public.is_admin(public.get_user_email())
);
-- ============================================
...
-- ============================================
-- SELECT: Admins can read all admin users, users can read their own
CREATE POLICY "admin_users_select_admin_or_own" ON public.admin_users FOR
SELECT USING (
    -- Avoid calling is_admin() here to prevent recursion
    email = COALESCE(
      current_setting('request.jwt.claims', true)::jsonb->>'email',
      ''
    ) -- Note: Super-admins can still see everything via service_role or if we add a non-recursive admin check
  );
-- INSERT: Only admins can create admin users (via Edge Function with service role)
CREATE POLICY "admin_users_insert_admin" ON public.admin_users FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- UPDATE: Only admins can update admin users
CREATE POLICY "admin_users_update_admin" ON public.admin_users FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete admin users
CREATE POLICY "admin_users_delete_admin" ON public.admin_users FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- REGISTRATION_REQUESTS POLICIES
-- ============================================
-- SELECT: Only admins can read registration requests
CREATE POLICY "registration_requests_select_admin" ON public.registration_requests FOR
SELECT USING (public.is_admin(public.get_user_email()));
-- INSERT: Public can submit registration requests
CREATE POLICY "registration_requests_insert_public" ON public.registration_requests FOR
INSERT WITH CHECK (TRUE);
-- UPDATE: Only admins can update registration requests
CREATE POLICY "registration_requests_update_admin" ON public.registration_requests FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete registration requests
CREATE POLICY "registration_requests_delete_admin" ON public.registration_requests FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- ORDERS POLICIES
-- ============================================
...
CREATE POLICY "orders_insert_public_or_admin" ON public.orders FOR
INSERT WITH CHECK (TRUE);
-- UPDATE: Business owners can update orders for their business, admins can update any
CREATE POLICY "orders_update_owner_or_admin" ON public.orders FOR
UPDATE USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "orders_delete_admin" ON public.orders FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
...
CREATE POLICY "appointments_select_owner_or_admin" ON public.appointments FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "appointments_insert_public_or_admin" ON public.appointments FOR
INSERT WITH CHECK (TRUE);
-- UPDATE: Business owners can update appointments for their business, admins can update any
CREATE POLICY "appointments_update_owner_or_admin" ON public.appointments FOR
UPDATE USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "appointments_delete_owner_or_admin" ON public.appointments FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.businesses
    WHERE businesses.id = appointments.business_id
...
CREATE POLICY "support_tickets_select_owner_or_admin" ON public.support_tickets FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "support_tickets_insert_owner_or_admin" ON public.support_tickets FOR
INSERT WITH CHECK (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "support_tickets_update_owner_or_admin" ON public.support_tickets FOR
UPDATE USING (
    EXISTS (
      SELECT 1
      FROM public.businesses
...
CREATE POLICY "support_tickets_delete_admin" ON public.support_tickets FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- ANNOUNCEMENTS POLICIES
-- ============================================
-- SELECT: Public can read announcements
CREATE POLICY "announcements_select_public" ON public.announcements FOR
SELECT USING (TRUE);
-- INSERT: Only admins can create announcements
CREATE POLICY "announcements_insert_admin" ON public.announcements FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- UPDATE: Only admins can update announcements
CREATE POLICY "announcements_update_admin" ON public.announcements FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete announcements
CREATE POLICY "announcements_delete_admin" ON public.announcements FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- APP_SETTINGS POLICIES
-- ============================================
-- SELECT: Public can read app settings (for public configuration)
CREATE POLICY "app_settings_select_public" ON public.app_settings FOR
SELECT USING (TRUE);
-- INSERT: Only admins can create app settings
CREATE POLICY "app_settings_insert_admin" ON public.app_settings FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- UPDATE: Only admins can update app settings
CREATE POLICY "app_settings_update_admin" ON public.app_settings FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete app settings
CREATE POLICY "app_settings_delete_admin" ON public.app_settings FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
-- PAGE_CONTENT POLICIES
-- ============================================
-- SELECT: Public can read page content
CREATE POLICY "page_content_select_public" ON public.page_content FOR
SELECT USING (TRUE);
-- INSERT: Only admins can create page content
CREATE POLICY "page_content_insert_admin" ON public.page_content FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- UPDATE: Only admins can update page content
CREATE POLICY "page_content_update_admin" ON public.page_content FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete page content
CREATE POLICY "page_content_delete_admin" ON public.page_content FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
...
CREATE POLICY "blog_comments_select_public" ON public.blog_comments FOR
SELECT USING (TRUE);
...
CREATE POLICY "blog_comments_insert_authenticated" ON public.blog_comments FOR
...
CREATE POLICY "blog_comments_update_admin" ON public.blog_comments FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete comments
DROP POLICY IF EXISTS "blog_comments_delete_admin" ON public.blog_comments;
CREATE POLICY "blog_comments_delete_admin" ON public.blog_comments FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
...
CREATE POLICY "admin_activity_logs_select_admin" ON public.admin_activity_logs FOR
SELECT USING (public.is_admin(public.get_user_email()));
-- INSERT: Only admins can insert logs
DROP POLICY IF EXISTS "admin_activity_logs_insert_admin" ON public.admin_activity_logs;
CREATE POLICY "admin_activity_logs_insert_admin" ON public.admin_activity_logs FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
-- DELETE: Only admins can delete logs
DROP POLICY IF EXISTS "admin_activity_logs_delete_admin" ON public.admin_activity_logs;
CREATE POLICY "admin_activity_logs_delete_admin" ON public.admin_activity_logs FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
...
CREATE POLICY "email_notifications_log_select_admin" ON public.email_notifications_log FOR
SELECT USING (public.is_admin(public.get_user_email()));
...
CREATE POLICY "email_notifications_log_insert_admin" ON public.email_notifications_log FOR
INSERT WITH CHECK (public.is_admin(public.get_user_email()));
...
CREATE POLICY "email_notifications_log_update_admin" ON public.email_notifications_log FOR
UPDATE USING (public.is_admin(public.get_user_email())) WITH CHECK (public.is_admin(public.get_user_email()));
...
CREATE POLICY "email_notifications_log_delete_admin" ON public.email_notifications_log FOR DELETE USING (public.is_admin(public.get_user_email()));
-- ============================================
