...
const DealCard: React.FC<DealCardProps> = ({ deal, businessName, businessSlug }) => {
  const formatPrice = (price?: number) => {
    if (price === undefined) return null;
    return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(price);
  };

  const hasPricing = deal.originalPrice !== undefined && deal.dealPrice !== undefined;
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col h-full transition-transform transform hover:-translate-y-1">
      <Link to={`/business/${businessSlug}#deals`} className="block">
        <div className="relative">
          <img
            ...
            src={getOptimizedSupabaseUrl(deal.imageUrl || 'https://picsum.photos/seed/deal/400/300', { width: 400, quality: 75 })}
            alt={deal.title}
          />
      ...
      <div className="bg-gray-50 p-3 border-t border-gray-100 text-xs text-gray-500">
        <p className="truncate">
          ...
          <Link to={`/business/${businessSlug}`} className="text-secondary hover:underline">{businessName}</Link>
        </p>
        {deal.endDate && (
          <p className="mt-1"><span className="font-semibold">Hạn cuối:</span> {new Date(deal.endDate).toLocaleDateString('vi-VN')}</p>
        )}
      </div>
    </div>
  );
