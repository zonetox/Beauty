...
export const AdminProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchAdminUsers = useCallback(async () => {
    if (!isSupabaseConfigured) {
        ...
        return DEV_ADMIN_USERS; // Return for immediate use
    }
  ...
  useEffect(() => {
    const handleAuthChange = async (allAdmins: AdminUser[], user: User | null) => {
        ...
        if (devLoginId) {
            ...
            if (devUser) {
                ...
                setLoading(false);
                return; // Dev login is active, stop here.
            } else {
                localStorage.removeItem(DEV_LOGIN_KEY); // Clean up invalid dev login
            }
        }
    ...
    if (isSupabaseConfigured) {
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
            setLoading(true);
            await handleAuthChange(adminUsers, session?.user ?? null);
        });
