...
function main() {
  console.log('🔄 Syncing Environment Variables from Vercel...\n');
  ...
  if (legacy.length > 0) {
    console.log('⚠️  Legacy Variables (should be removed):');
    legacy.forEach(key => console.log(`   - ${key}`));
    console.log('');
  }

  if (Object.keys(found).length > 0) {
    console.log('✅ Valid Variables Found:');
  ...
  if (unknownVars.length > 0) {
    console.log('❓ Unknown Variables (not in requirements):');
  ...
  if (missing.length === 0 && invalid.length === 0) {
    ...
    console.log(`✅ Created .env.local with ${Object.keys(found).length} variables\n`);
    ...
    if (legacy.length > 0) {
      console.log('⚠️  Recommendation: Remove legacy variables from Vercel:');
      legacy.forEach(key => console.log(`   - ${key}`));
      console.log('');
    }

    if (unknownVars.length > 0) {
      console.log('⚠️  Note: Unknown variables found (may be from other projects)');
      console.log('');
    }

    console.log('🎉 Sync complete! You can now run: npm run dev');
  } else {
    console.log('❌ Sync failed. Please fix the issues above and try again.');
    process.exit(1);
  }
}
